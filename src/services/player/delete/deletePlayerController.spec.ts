...
describe("Delete player controller", () => {
  ...
  it("should be able to delete a player", async () => {
    const user = await request(app).post("/users").send({
      username: "test-integration-delete-player",
      email: "test-integration-delete-player@example.com",
      password: "test123",
    });
    ...
    const response = await request(app)
      .delete(`/players/${player.body.id}`)
      .set("Authorization", `Bearer ${req.body.token}`);

    expect(response.status).toBe(201);
  });

  it("should not be able to delete a player if player not found", async () => {
    const user = await request(app).post("/users").send({
      username: "test-integration-delete-player",
      email: "test-integration-delete-player@example.com",
      password: "test123",
