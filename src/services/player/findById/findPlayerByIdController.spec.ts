...
describe("Find player by id controller", () => {
  let position: request.Response;

  it("should be able to find a player by id", async () => {
    const user = await request(app).post("/users").send({
      username: "test-integration-find-player-by-id",
      email: "test-integration-find-player-by-id@example.com",
      password: "test123",
    });
    ...
    const response = await request(app)
      .get(`/players/${player.body.id}`)
      .set("Authorization", `Bearer ${req.body.token}`);

    expect(response.status).toBe(201);
  });

  it("should not be able to find a player by id if player not found", async () => {
    const user = await request(app).post("/users").send({
      username: "test-integration-find-player-by-id",
      email: "test-integration-find-player-by-id@example.com",
      password: "test123",
